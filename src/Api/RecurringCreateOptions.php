...
/**
 * Payload für POST /recurrings.
 *
 * Items können beim Anlegen direkt eingebettet werden (über addItem()) –
 * spätere Änderungen an Items müssen über RecurringItemsApi laufen.
 *
 * Doku: https://www.billomat.com/api/abo-rechnungen/
 */
final class RecurringCreateOptions
{
    ...
    public function addItem(RecurringItemCreateOptions $item): self
    {
        $this->items[] = $item;
    ...
    /**
     * @return list<RecurringItemCreateOptions>
     */
        ...
        if ([] !== $this->items) {
            $data['recurring-items'] = [
                'recurring-item' => array_map(
                    static fn (RecurringItemCreateOptions $item): array => $item->toArray(),
                    $this->items,
                ),
            ];
        }
